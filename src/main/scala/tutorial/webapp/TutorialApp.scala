...
object TutorialApp extends JSApp{
  ...
  contentArray += "public void developerLoop() throws ImportantUpdatesException"
  ...
  contentArray += "Good Reads"
  contentArray += "Challenged Accepted"
  ...
  contentArray += "Last week's winner is..."
  ...
  def buildBody(): String = {
    val body = START_BODY +
               H2_START + contentArray(0) + H2_END +
               P_START + contentArray(1) +
               EM_START + contentArray(2) + EM_END +
               contentArray(3) + P_END +
               H2_START + contentArray(4) + H2_END +
               UL_START +
               LI_START + contentArray(5) + LI_END +
               LI_START + contentArray(6) + LI_END +
               LI_START + contentArray(7) + LI_END +
               LI_START + contentArray(8) + LI_END +
               H2_START + contentArray(9) + H2_END +
               LINKS +
               H2_START + contentArray(10) + H2_END +
               P_START + contentArray(11) + P_END +
               P_START + contentArray(12) +
               LINKS2 +
               contentArray(13) + P_END +
               H3_START + contentArray(14) + H3_END +
               P_START + contentArray(15) + P_END +
               PRE_START + contentArray(16) + PRE_END +
               P_START + contentArray(17) + P_END +
  ...
  def setupUI3(): Unit = {
    //jQuery("body").append(content1)
    //jQuery("body").append(buildBody())
    //jQuery("body").append(content1)
    //dom.setInterval(() => {jQuery(".wrapper").append("<p>click it...</p>")}, 1000)
    //jQuery("body").append(content1)
    jQuery("body").empty().append(buildBody())
    dom.setInterval(() => sneakySneaky(), 1000)
  }
  ...
  def sneakySneaky(): Unit = {
    ...
    contentArray(contentIndex) = swapChars(contentArray(contentIndex),
                                           indexMaker.nextInt(contentArray(contentIndex).length),
                                           indexMaker.nextInt(contentArray(contentIndex).length))
    jQuery("body").empty().append(buildBody())
  }
  ...
  def main(): Unit = {
    jQuery.apply(setupUI2())
    jQuery.apply(setupUI3())
    //content1 = content1.charAt(1)
    //jQuery(setupUI2(content4) _)
  }
}
